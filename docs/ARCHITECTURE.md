...
**A compound refuses only what croft cannot READ.** `preLaunchTask` (as a string), `hidden`, `group` and `order` are all honoured; a `preLaunchTask` that is not a string asks for a task croft cannot name, so it is refused rather than launched with the task quietly skipped; what remains in `unsupported_keys` is a value croft cannot read — a `presentation` that is not an object, or a `hidden`, `group` or `order` that is not a bool, string or number. A `presentation` that is `null`, `false`, `""` or empty is not recorded — the first three ask for nothing, and an empty block asks only to be declared, which the sort reads by placing it ahead of a row carrying no block at all. Inside the block only `null` is carved out. A mistyped KEY is ignored rather than refused, so whatever VS Code adds next is not rejected. Those keep refusing because `{"hidden": "true"}` is a typo, and launching it unhidden with no signal is worse than the refusal. `stopAll` depends on the member count. It decides whether ending one session ends the others, so a one-member compound has no others and neither value can change what it does — it is not recorded at either. A multi-member compound behaves as `stopAll: false`: one member ending leaves its siblings running, which is VS Code's own default, so omitting the key or writing `false` gets exactly what it asked for. `stopAll: true` asks for a teardown croft does not implement, and at two or more members it is **refused** rather than launched as its opposite — recorded in `unsupported_keys` and named in the message, the same as a malformed key. Documenting the mismatch would not have stopped the launch; implementing the teardown is the follow-up to #310.
